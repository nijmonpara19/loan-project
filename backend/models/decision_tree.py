...
class DecisionTreePredictor(BaseModelPredictor):
    ...
    def __init__(self, pkl_path: str = None):
        ...
        if pkl_path is None:
            ...
            candidates = [
                project_root / "models" / "DecisionTreeModel.pkl",
                project_root / "notebooks-containing-models" / "DecisionTreeModel.pkl",
                current_dir.parent / "models" / "DecisionTreeModel.pkl",
                project_root / "DecisionTreeModel.pkl",
            ]
            self.pkl_path = next((p for p in candidates if p.exists()), candidates[1])
        else:
